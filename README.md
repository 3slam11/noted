<div align="center">
   
<p>
<img src="https://github.com/user-attachments/assets/70308c90-dea1-471d-8b5f-276209647fb2" width="20%"/>
<p/>

**Track Your Entertainment Journey**   

[![Platform](https://img.shields.io/badge/Platform-Android-green)]() [![Platform](https://img.shields.io/badge/Platform-Windows-11)]() ![IzzyOnDroid](https://img.shields.io/endpoint?&logo=data:image/png;base64,&url=https://apt.izzysoft.de/fdroid/api/v1/shield/com.app.noted&label=IzzyOnDroid) ![Latest version](https://img.shields.io/github/v/release/3slam11/noted?color=2596be&label=latest) [![License](https://img.shields.io/github/license/3slam11/noted)](https://github.com/3slam11/noted/blob/main/LICENSE) ![Downloads count](https://img.shields.io/github/downloads/3slam11/noted/total?color=1976d2)

</div>

Noted is a tracker that helps you organize movies, TV series, books, and games across **To-Do** and **Finished** lists. Discover new titles, and analyze your habits—all in one app.  

<p float="left">
  <img src="https://github.com/user-attachments/assets/d31e6a87-9f18-4e14-ae26-013d9ffbd3cb" width="30%"/>
  <img src="https://github.com/user-attachments/assets/11cf5d74-6c8c-411f-ad25-2730960891f1" width="30%"/>
  <img src="https://github.com/user-attachments/assets/35fc8aef-c70c-47c5-a661-f424d5d77b96" width="30%"/>
</p>

 <p float="left">
<img src="https://github.com/user-attachments/assets/b5a85f26-9cae-433e-8724-d883de2cf3c0" width="30%"/>
<img src="https://github.com/user-attachments/assets/42e3a096-0544-4aa0-a658-aa5857a27362" width="30%"/
   </p>

## Features  
- **Track Everything**  
  - Movies, TV shows, books, and games.  
  - To-Do (plan to watch/read/play) and Finished lists.  
- **Monthly Rollover**  
  - Automatically archives completed items and lets you migrate unfinished ones.  
- **Discover**  
  - Search using [TMDB](https://www.themoviedb.org/), [RAWG](https://rawg.io/), and [Google Books](https://developers.google.com/books) APIs.  
- **Insights**  
  - View statistics (e.g., monthly progress, favorite genres).  
- **Customize**  
  - **Themes**: Auto (monthly) or manual.  
  - **Localization**: Multilingual support via Slang.  
- **Backup & Restore**: Export and import data to/from JSON.  

## API Keys  
Noted uses:  
- [TMDB](https://developers.themoviedb.org/3) (movies/TV)  
- [RAWG](https://rawg.io/apidocs) (games)  
- [Google Books](https://developers.google.com/books) (books)  

**Default keys are rate-limited.** For more usage, add your own in *Settings > Change API*.  

## Download  
<p align="center">
  <a href='https://apt.izzysoft.de/packages/com.app.noted'>
    <img src='https://gitlab.com/IzzyOnDroid/repo/-/raw/master/assets/IzzyOnDroid.png' width='200' alt='Get it on IzzyOnDroid'>
  </a>
   
  <a href='https://openapk.net/noted/com.app.noted/'>
    <img src='https://www.openapk.net/images/openapk-badge.png' width='200' alt='Get it on OpenAPK'>
  </a>
</p>
